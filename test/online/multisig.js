...
const remote = (require('./connector'))({structured:false});
...
describe.skip('利用托管合约实现多签流程', () => {
    it('Alice建立一个 2/2 多签合约实例', async () => {
        //注册合约
        let msg = await remote.execute('sc.register', [{'type':'multisig', 'm':2, 'n':2}]);
        assert(msg.dst);
        //设置合约地址
    ...
    it('Alice建立本地账户', async () => {
        ...
        //生成新地址, 该地址既是多签钱包的标识，也是多签合约中Alice的通信地址
        let msg = await remote.execute('address.create', ['alice']);
        assert(msg.address);
        env.address['alice'] = msg.address;
        //系统转账
        await remote.execute('tx.send', [env.address['alice'], 500000000]);
        //上链
        await remote.execute('miner.generate.admin', [1]);
        //创建本地多签钱包
        msg = await remote.execute('wallet.create', [env.address['alice'],'multisig',2,2,,,,true]);
    ...
    it('Bob建立本地账户', async () => {
        ...
        let msg = await remote.execute('address.create', ['bob']);
        assert(msg.address);
        env.address['bob'] = msg.address;
        //系统转账
        await remote.execute('tx.send', [env.address['bob'], 500000000]);
        //上链
        await remote.execute('miner.generate.admin', [1]);
        //创建本地多签钱包
        msg = await remote.execute('wallet.create', [env.address['bob'],'multisig',2,2,,,,true]);
        assert(msg.account.accountKey);
    ...
    it('向多签收款地址转账，Alice和Bob的多签钱包将收到对应UTXO', async () => {
        //转账
    ...
    it('Alice消耗多签钱包的UTXO，向Robin转账', async () => {
    ...
    it('Bob查询多签交易列表，逐项补签后广播，Robin收到款项', async () => {
        ...
        for(let trans of list) {
            await remote.execute('tx.mstran.sign', [trans.txid]);
        }
