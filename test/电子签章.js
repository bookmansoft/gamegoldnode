...
describe('GIP0024 电子签章', function() {
                ...
                switch(msg.content.type) {
                    case 'agreement': {
                        ...
                        let ret = await remote.execute('ca.issue.public', [
                            ...
                            msg.content.payload.data.pubkey,          //目标地址公钥
                            env.bob.name,                             //见证账户
                        ]);
                        assert(ret.erid);
    ...
    it('个人签发：Bob为Alice签发证书', async () => {
        let ret = await remote.execute('ca.issue.public', [
            {
                name: '',               //name
                hash: env.content,      //content hash
                height: 0,              //有效期，填0表示使用默认值
            },
            env.bob.address,            //见证地址
            env.alice.pubkey,           //存证地址
            env.bob.name,               //见证地址归属账户
        ]);
        assert(ret.erid);
